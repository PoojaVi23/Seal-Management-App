...
class LeaveApplicationStatus extends StatefulWidget {
  const LeaveApplicationStatus({Key? key}) : super(key: key);

  @override
  _LeaveStatusState createState() => _LeaveStatusState();
}

class _LeaveStatusState extends State<LeaveApplicationStatus> {
  ...
  Future<void> changeLeaveStatus(String leaveId, String status) async {
    try {
      ...
      final response = await http.post(
        Uri.parse('$API_URL/Mobile_flutter_api/change_leave_status'),
  ...
  Future<List<dynamic>> fetchLeaveData() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('$API_URL/Mobile_flutter_api/get_leaves'),
        headers: {"Accept": "application/json"},
        body: {
          'uuid': _uuid,
          'user_id': _username,
