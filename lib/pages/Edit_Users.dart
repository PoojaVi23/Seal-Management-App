...
class EditUsers extends StatefulWidget {
  ...
  EditUsers({required this.userId});
...
class _EditUsersState extends State<EditUsers> {
  ...
  void _initPreferences() async {
    ...
    setState(() {
      _loggedInUsername = _prefs?.getString('username');
  ...
  void _fetchUserData() async {
    try {
      ...
      // API endpoint URL
      var apiUrl = '$API_URL/Mobile_flutter_api/get_single_user';
      ...
      if (response.statusCode == 200) {
        ...
        // Check if jsonResponse and user_data are not null
        if (jsonResponse != null && jsonResponse['user_data'] != null) {
          var userData = jsonResponse['user_data'];

          // More debugging print statements
          setState(() {

            selectedUserType = userTypeMap[userData['user_type']] ?? 'Select User Type';
            ...
            _originalPassword = userData['password'] ?? '';

            isActiveYes = userData['active_user'] == '1';
            isActiveNo = userData['active_user'] != '1';

            isMobileLoginYes = userData['allowed_mobile_login'] == '1';
            isMobileLoginNo = userData['allowed_mobile_login'] != '1';

            hasAccessSealDataYes = userData['access_seal_data'] == '1';
            hasAccessSealDataNo = userData['access_seal_data'] != '1';

            isSenderYes = userData['sender'] == '1';
            isSenderNo = userData['sender'] != '1';

            isReceiverYes = userData['receiver'] == '1';
            isReceiverNo = userData['receiver'] != '1';

            isReadOnlyUserYes = userData['readonly'] == '1';
            isReadOnlyUserNo = userData['readonly'] != '1';

            hasAccessScrapDataYes = userData['access_scrap_data'] == '1';
            hasAccessScrapDataNo = userData['access_scrap_data'] != '1';

            hasAccessGPSYes = userData['access_gps_module'] == '1';
            hasAccessGPSNo = userData['access_gps_module'] != '1';

            selectedPlantIds = Set<String>.from(userData['plant'].split(','));
            selectedMaterialIds  = Set<String>.from(userData['material'].split(','));

            uuIDController.text =  userData['uuid'] ?? '';

          });
        } else {
          // Handle case when user_data is null
          print('Error: user_data is null');
        }
      } else {
  ...
  void dropdown() async {
    try {
      ...
      // API endpoint URL
      var apiUrl = '$API_URL/Mobile_flutter_api/get_dropdown_data';
      // API request parameters
      var params = {
        'uuid': _uuid,
        'user_id': _username,
  ...
  void _saveUserData() async {
    ...
    try {
      ...
      // API endpoint URL
      var apiUrl = '$API_URL/Mobile_flutter_api/add_edit_user_data';
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['status'] == "success") {
          ...
          // Check if the username or password has changed
          bool usernameChanged = usernameController.text != _originalUsername;
          bool passwordChanged = passwordController.text != _originalPassword;

          print("bhasdaf : $usernameChanged");
          ...
          // If the username or password has changed, log out the user
          if ((usernameChanged || passwordChanged) && _username == _originalUsername) {
            SharedPreferences prefs = await SharedPreferences.getInstance();
            await prefs.clear();
            ...
            // If no change, just pop the screen
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: ListView(
          children: [
            _buildCreativeCard('Edit User Details'),
            SizedBox(height: 12),
