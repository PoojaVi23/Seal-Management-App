...
class Emp_Tracker extends StatefulWidget {

  final String? selectedEmployeeName;

  Emp_Tracker({Key? key, required this.selectedEmployeeName}) : super(key: key);


  @override
  State<Emp_Tracker> createState() => EmployeeTrackersState();
}

class EmployeeTrackersState extends State<Emp_Tracker> {
  ...
  void initState() {
    ...
    _fetchDropdownData().then((_) {
      // Set selectedEmployeeId after _fetchDropdownData() completes
      setState(() {
        selectedEmployeeId = widget.selectedEmployeeName!;
      });
    });
  }
  ...
  Future<void> _fetchUserData() async {
    ...
    final url = '$API_URL/Mobile_flutter_api/get_user_lat_long';
    final response = await http.post(
      Uri.parse(url),
      body: {
        'uuid': _uuid,
